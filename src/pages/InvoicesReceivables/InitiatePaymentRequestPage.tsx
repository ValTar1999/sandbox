...
const SmartCollectTooltipContent = () => (
  <div className="w-[344px] p-4 bg-gray-900 rounded-lg shadow-lg flex flex-col gap-1">
    ...
    <p className="text-sm leading-5 text-gray-100">
      Embedded payment solution that allows your business to bill and collect
      payments from businesses or consumers
    </p>
    <a
      href="#"
    ...
    >
      ...
      <Icon
        icon="arrow-right"
        className="w-3 h-3 text-blue-500 group-hover:text-blue-300"
      />
    </a>
  </div>
);
...
const PaymentCollectionSelect = ({
  ...
  const selectedOption = paymentCollectionOptions.find(
    (o) => o.value === value
  );
  return (
  ...
  <div className="flex flex-col gap-1">
    ...
    {error && (
      <span className="text-sm text-red-500">
        Please add and select an account for payment initiation.
      </span>
    )}
  </div>
);
  ...
  return (
    ...
    >
      ...
      <input
        ...
        type="text"
        placeholder={
          selectedContacts.length === 0 ? 'Email address or phone number' : ''
        }
        value={contactQuery}
...
const InitiatePaymentRequestPage = () => {
  ...
  const [isSmartCollectModalOpen, setIsSmartCollectModalOpen] = useState(false);
      ...
      footer={
        <div className="flex flex-col gap-2 w-full">
          <div className="flex items-center flex-wrap justify-end w-full">
            <span className="text-sm leading-5 text-gray-700 mr-4 font-medium">
              Payment Collection
            </span>
            ...
            <Button size="md" variant="primary" onClick={handleInvoiceClick}>
              {paymentCollection === 'charge' ? 'Charge' : 'Invoice'}:{' '}
              {receivable.amount}
            </Button>
            <div className="ml-2">
              <DropdownCalendar
    ...
    >
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-22 w-full ">
              <div
                className={clsx(
                  'w-full',
                  paymentCollection === 'charge' && 'max-w-[572px]'
                )}
              >
                ...
                {paymentCollection === 'charge' ? (
                  <div className="text-sm leading-5 text-gray-500">
                    Select customer account details you want to make the charge
                    for this invoice. Transaction will be initiated immediately.
                    Ensure you have consent from the customer to charge them on
                    this account.
                  </div>
                ) : (
                  <div className="text-sm leading-5 text-gray-500">
                    Provide your payor&apos;s email address and/or phone number
                    where the{' '}
              ...
              {paymentCollection === 'invoice' && (
                <div className="w-full">
                  ...
                  <div className="text-sm leading-5 text-gray-500">
                    This account serves as the destination for funds charged
                    from the customer for ACH receivables.
                  </div>
                </div>
              )}
              {paymentCollection === 'charge' && selectedAccountOnFile && (
                <div className="w-full">
                  ...
                  <div className="text-sm leading-5 text-gray-500">
                    This account serves as the destination for funds charged
                    from the customer for credit and debit card transactions.
                  </div>
                </div>
              )}
            </div>
                ...
                {paymentCollection === 'invoice' ? (
                  ...
                  >
                    <WrapSelect
                      ...
                      options={(accountsOnFile.length > 0
                        ? accountsOnFile
                        : chargeSourceAccountDemo
                      ).map((acc, index) => {
                        const lastFour =
                        ...
                        return {
                          ...
                          iconImageClassName: isCard
                            ? 'h-4 w-5 flex-shrink-0'
                            : undefined,
                          badge: isFirst ? 'SMART Collect' : undefined,
                          badgeSecondary:
                            (isCard && isDefault) || isFirst
                              ? 'Default'
                              : undefined,
                          badgeSecondaryTooltip:
                            (isCard && isDefault) || isFirst ? (
                              <>
                                This is the default payment method that the
                                customer has set through{' '}
                    ...
                    <AddCustomerAccountSelect
                      ...
                      onAddCardDetails={() =>
                        setIsAddCardDetailsModalOpen(true)
                      }
                    />
                  </div>
                )}
                  ...
                  <WrapSelect
                    ...
                    options={accountsOnFile.map((acc) => {
                      ...
                      return {
                        ...
                        value: acc.id,
                        description:
                          isCard && acc.expiry ? acc.expiry : undefined,
                        descriptionPosition: isCard
                          ? ('inline' as const)
                          : undefined,
                        icon: !isCard ? 'library' : undefined,
                        iconImageSrc: isCard ? MastercardIcon : undefined,
                        ...
                        iconImageClassName: isCard
                          ...
                          : undefined,
                        badge:
                          isCard && selectedAccountOnFile === acc.id
                            ? 'Default'
                            : undefined,
        ...
        <div className="flex flex-col gap-6">
          {receivable.receivableSummary &&
            receivable.receivableSummary.length > 0 && (
              <Accordion title="Receivable Summary">
                  ...
                  <thead className="border-b border-dashed border-gray-200">
                    <tr className="text-gray-500 text-xs tracking-wider uppercase">
                      <th className="font-medium text-left py-2 px-6">
                        ITEM ({receivable.receivableSummary.length})
                      </th>
                      <th className="font-medium text-left py-2 px-6">
                        QUANTITY
                      </th>
      ...
      <SmartCollectDetailsModal
        ...
        customerAccount={(() => {
          ...
          if (!source) return '';
          const last4 =
            source.accountNumber.replace(/\s/g, '').slice(-4) || '----';
