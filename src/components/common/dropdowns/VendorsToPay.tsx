...
import {
  ...
  Payment,
  PayableItem,
} from '../../../pages/BillsPayables/data';

const SUPPORTED_BULK_PAYMENT_METHODS = [
  'ach',
  'rtp',
  'smart exchange',
  'wire',
...
const VendorsToPay: React.FC<VendorsToPayProps> = ({
  ...
  const [bulkPaymentByVendor, setBulkPaymentByVendor] = useState<
    Record<string, boolean>
  >({});
  const [unsupportedBulkPaymentModal, setUnsupportedBulkPaymentModal] =
    useState<{
      open: boolean;
  ...
  const normalizeBankAccount = useCallback(
  ...
  const isPaymentMethodSupported = useCallback((method: string): boolean => {
    ...
    return SUPPORTED_BULK_PAYMENT_METHODS.some(
      (m) =>
  ...
  return (
    <div>
      <div className="flex items-center justify-between border-b border-gray-300 pb-4">
        <div className="flex flex-col gap-1">
          ...
          <div className="text-sm text-gray-500">
            {totalVendors} vendors with a total of {totalVendorsPayables}{' '}
            payables.
          </div>
        </div>
      ...
      <div>
        {vendors.map((vendor) => {
          const vendorTotalAmount = calculateTotalAmount(
            vendor.payables
          ).toFixed(2);
                ...
                <div className="flex items-center gap-5">
                  <Icon
                    ...
                    className={clsx(
                      ...
                      hasPaymentMethod && openVendors.includes(vendor.name)
                        ? 'rotate-90'
                        : 'rotate-0'
                    )}
                  />
                ...
                <div className="grid w-full grid-cols-[196px_auto_auto_1fr] items-center">
                  <div className="flex flex-col gap-1 text-sm">
                    ...
                    <div className="text-gray-500">
                      {vendor.payables.length} payables
                    </div>
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex items-center gap-2 px-4">
                      <Toggle
                        checked={
                          vendorBulkPaymentProp?.[vendor.name] ??
                          bulkPaymentByVendor[vendor.name] ??
                          false
                        }
                        onChange={(checked) => {
                          const currentMethod = (
                          ...
                          const isSupported =
                            isPaymentMethodSupported(currentMethod);
                          const isSmartDisburse = currentMethod
                            .toLowerCase()
                            .includes('smart disburse');
                      ...
                      <Tooltip placement="top">
                        ...
                        >
                          <Icon
                            ...
                            className="w-4.5 h-4.5 text-gray-400 cursor-help"
                          />
                        </TooltipTrigger>
                    ...
                    {hasPaymentMethod ? (
                      <div className="inline-flex items-center gap-2 rounded-md border border-gray-200 bg-gray-100 py-0.5 px-2.5">
                        <div className="text-sm font-medium text-gray-900">
                          ...
                          {(() => {
                            const contacts =
                              vendorPaymentContactsProp?.[vendor.name];
                            const email =
                              vendorPaymentEmailsProp?.[vendor.name];
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="text-sm font-medium text-gray-900">
                      ${formatAmount(vendorTotalAmount)}{' '}
                      <span className="text-gray-500">USD</span>
                    </div>
                  </div>
                </div>
                ...
                >
                  {vendor.payables.map((payable) => {
                    ...
                    const selectedBankAccount = (
                      vendorBankAccountsProp?.[vendor.name]?.label || ''
                    ).trim();

                    const normalizedOriginal =
                      normalizeBankAccount(originalBankAccount);
                    const normalizedSelected =
                      normalizeBankAccount(selectedBankAccount);
                      ...
                      >
                        ...
                        <div className="grid w-full grid-cols-[423px_auto_auto_1fr] p-4 gap-4 items-center">
                          <div className="flex flex-col">
                            <div className="uppercase font-medium text-gray-900">
                              {payable.id}
                            </div>
                            <div className="flex items-center gap-2">
                              {bankAccountChanged ? (
                                <>
                                  <div className="text-sm text-gray-500 line-through text-nowrap">
                                    {originalBankAccount}
                                  </div>
                                  <div className="text-sm text-gray-900 text-nowrap">
                                    {selectedBankAccount}
                                  </div>
                                  <Badge
                                    size="xs"
                          ...
                          <div className="flex items-center gap-1 font-medium">
                            <div className="text-xs uppercase text-gray-500 mt-0.5">
                              Due:
                            </div>
      ...
      <UnsupportedBulkPaymentMethodModal
        ...
        onChangePaymentMethod={() => {
          if (unsupportedBulkPaymentModal.vendorName) {
            onSelectPaymentMethodClick?.(
              unsupportedBulkPaymentModal.vendorName
            );
          }
        }}
        onDiscard={() => {
